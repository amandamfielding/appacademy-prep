  ...
  def two_sum
    ...
    each_with_index do |num1,idx1|
      each_with_index do |num2,idx2|
        next if idx1 == idx2
        zero_sum_pairs << [idx1,idx2] if num1 + num2 == 0 && !zero_sum_pairs.include?([idx2,idx1])
      end
    end
    zero_sum_pairs
  end
...
class Array
  def median
    return nil if empty?
    self.sort!.length.odd? ? self[self.length/2] : (self[(self.length-1)/2] + self[self.length/2]) / 2.0
  end
end
...
class Array
  def my_transpose
    ...
    transposed_array = Array.new(dimension) {Array.new(dimension)}
